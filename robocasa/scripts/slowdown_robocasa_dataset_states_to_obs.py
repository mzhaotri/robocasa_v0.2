...
def extract_trajectory(
    ...
    print("extracting trajectory with done_mode={}".format(done_mode))
    """
    ...
    slowed_trajectory = np.concatenate(
        [slowed_trajectory, zero_base], axis=1
    )  # concatenate the zero base to the slowed trajectory
    ...
    traj = dict(
        ...
        actions=actions,
        actions_abs=[],
        original_states=np.array(states),
        states=[],
    ...
    for t in range(traj_len):
        ...
        # obs = deepcopy(env._get_observations())
...
def write_traj_to_file(
        ...
        while (total_run.value < (processes)) or not mul_queue.empty():
            if not mul_queue.empty():
                ...
                try:
                    ...
                    print("traj action shape", traj["actions"].shape)
    ...
    # env_meta['env_name'] = 'CoffeeServeMug'
    # drop env name from env kwargs
...
def extract_multiple_trajectories_with_error(
    ...
    print("Process {}: starting with index {}".format(process_num, ind))
    ...
    while (not work_queue.empty()) and (ind != -1):
        try:
            ...
            # print("states", states)
            # # assert 0 ==1
            # print("EXTRACTING TRAJECTORY FOR EPISODE {}".format(ep))
...
def dataset_states_to_obs_multiprocessing(args):
    ...
    num_demos = len(demos)
    print("Number of demos to process: {}".format(num_demos))
    f.close()
    ...
    process1 = multiprocessing.Process(
        target=write_traj_to_file_replay_robocasa_demos,
        args=(
            args,
            output_path,
